export const Moves: {[k: string]: ModdedMoveData} = {
	...
	touchdown: {
		...
		shortDesc: "Def -1 on target. Damage *2 if target is hurt this turn.",
		pp: 10,
		priority: 0,
		flags: {protect: 1, mirror: 1, metronome: 1, bullet: 1},
		onPrepareHit(target, source, move) {
	...
	topple: {
		...
		category: "Physical",
		name: "Topple",
		desc: "The user repeatedly stomps the ground to topple and damage opposing Pokémon. This move reduces the target's Defense stat and may remove their held items.",
		shortDesc: "Hits adj. foes. Def -1 and 30% chance of Knock Off on foes.",
		pp: 15,
		priority: 0,
	...
	gyropulse: {
		...
		category: "Special",
		name: "Gyro-Pulse",
		desc: "The user's body spins to gather energy for its beam attack. This move lowers opposing Pokémon's Special Defense and eliminates the effects of such moves as Bind, Wrap, and Leech Seed.",
		shortDesc: "Removes binding effects and hazards for user. SpD -1 to targets.",
		pp: 15,
		priority: 0,
		...
		onAfterHit(target, pokemon, move) {
			if (!move.hasSheerForce) {
				if (pokemon.hp && pokemon.removeVolatile('leechseed')) {
					this.add('-end', pokemon, 'Leech Seed', '[from] move: Gyro-Pulse', '[of] ' + pokemon);
				}
				const sideConditions = ['spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'gmaxsteelsurge'];
				for (const condition of sideConditions) {
					if (pokemon.hp && pokemon.side.removeSideCondition(condition)) {
						this.add('-sideend', pokemon.side, this.dex.conditions.get(condition).name, '[from] move: Gyro-Pulse', '[of] ' + pokemon);
					}
				}
	...
	bouncyshield: {
		...
		category: "Status",
		name: "Bouncy Shield",
		desc: "The user protects itself from all attacks with springy wool. If a Pokémon attempts to make contact, they will be knocked away, and a different Pokémon will be dragged out.",
		shortDesc: "Protects from damaging moves. Attacker that makes contact is forced out.",
		pp: 10,
		priority: 4,
		flags: {noassist: 1, failcopycat: 1},
		stallingMove: true,
		volatileStatus: 'bouncyshield',
		onPrepareHit(target, source, move) {
			this.attrLastMove('[still]');
		...
		condition: {
			...
			onTryHit(target, source, move) {
				...
				if (this.checkMoveMakesContact(move, source, target)) {
					//Forces attacker out if it makes contact - Taken from Red Card item
	...
	butlerservice: {
		...
		category: "Status",
		name: "Butler Service",
		desc: "The user puts itself in service of its ally. Depending on the ally's condition, the user will raise their ally's Atk. or Sp. Atk. stat or restore their HP.",
		shortDesc: "Goes first. Atk.+1 or SpA.+1 to ally, or heal ally if wounded.",
		pp: 5,
		priority: 5,
		...
		onTry(source, target, move) {
			//Only a male Indeedee can use this move
			...
			if (source.species.name === 'Indeedee-F') {
				this.attrLastMove('[still]');
				this.add('-fail', source, 'move: Butler Service', '[forme]');
				this.add('-message', `${source.name} could not perform the correct service!`);
				return null;
			}
			//Wrong species or any other failure case
			this.hint("Only a Pokemon whose form is Indeedee (Male) can use this move.");
			this.attrLastMove('[still]');
			this.add('-fail', source, 'move: Butler Service');
			return null;
		},
		onTryHit(target, source, move) {
			if (target.hp * 2 <= target.maxhp) {
			...
			if (target.getStat('spa', false, true) > target.getStat('atk', false, true)) move.boosts = { spa: 1, };
		},
		secondary: null,
		boosts: {
			atk: 1,
